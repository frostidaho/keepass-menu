...
def make_item(label, db, keyfile, password, collection):
    def _set():
        ...
        print(msg.format(label, collection.get_label()))
...
def get_or_make_collection(bus, label):
    ...
    if coll:
        print('Collection {} exists'.format(label))
        return coll
    print('Creating collection {}'.format(label))
...
def get_item(label, collection):
    ...
    if li == 1:
        ...
        warnings.warn(msg.format(label), UserWarning)
        print(msg.format(label, collection.get_label()))
        return items[0]
